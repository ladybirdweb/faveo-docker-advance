...
if [[ $# -lt 8 ]]; then
    echo "Please Input all the necessary arguments."
    exit 1;
fi
...
echo "<h1>Obtaining SSL Certificates from Let's Encrypt</h1>" > $CUR_DIR/certbot/html/index.html
...
if [[ $? -eq 0 ]]; then
    ...
    echo "Unable to change the owership to $CUR_SIR/certbot. Please run the script with sudoer's privilege."
    sleep 3 && echo "Exiting..." 
    exit 1;
fi;
...
if [[ $? -eq 0 ]]; then
    ...
    echo "Database root password: $db_root_pw"
